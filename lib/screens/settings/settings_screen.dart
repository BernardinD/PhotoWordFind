...
class _ImportLimitTileState extends State<_ImportLimitTile> {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
          content: Text(value < 0
              ? 'Import selection set to Unlimited'
              : 'Import selection limit set to $value')),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return ListTile(
      ...
      subtitle: Text(
          current == null ? 'Loading...' : 'Current: ${_labelFor(current)}'),
      onTap: () async {
        final result = await showModalBottomSheet<int>(
          ...
          builder: (ctx) => SafeArea(
            child: ListView(
              ...
              children: [
                const ListTile(
                    title: Text('Select max images per import',
