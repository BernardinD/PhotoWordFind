...
crop_image.Image crop(
    crop_image.Image image, String filePath, ui.Size screenSize) {
  ...
  return crop_image.copyCrop(image,
...
File createCroppedImage(String filePath, Directory parent, ui.Size size) {
  ...
  croppedFile =
      crop_image.copyResize(croppedFile, height: croppedFile.height ~/ 3);

  // Save temp image
  String fileName = filePath.split("/").last;
  File tempCropped = File('${parent.path}/temp-$fileName');
  tempCropped
      .writeAsBytesSync(crop_image.encodeNamedImage(filePath, croppedFile)!);
